...
def request_upgrade():
  ...
  query = db.session.query(PassportRequest).filter(PassportRequest.user_id == body.user_id);
...
def list_requests():
  ...
  for key in request.args.keys():
    if (key in mapper.attrs):
      query = query.filter(getattr(PassportRequest, key) == request.args[key]);
  passportRequests = query.all();
  passports = [PassportRequestSchema.from_orm(x).dict() for x in passportRequests]
  print(passports);
  return {
    "passports": passports
  }, 200
