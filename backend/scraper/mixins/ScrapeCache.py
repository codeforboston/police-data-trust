"""
This module contains the ScrapeCache class, which is used to store and retrieve scraped data from a cache.
...
class ScrapeCache:
    ...
    def set_json(
        ...
        self.backend_client.set_json(key, value, table, expiration_time)  # type: ignores
...
class RedisClient:
    def __init__(self, host: str = "scraper-cache", port: int = 6379, db: int = 0):
        self.connection = redis.StrictRedis(host=host, port=port, db=db)

    def _encode_dict(self, value: dict[str, Any]) -> dict[str, str]:
        """Redis doesn't support storing dicts with non-string values, so we need to convert them to strings."""
    ...
    def set_json(
        ...
        Args:
            ...
            table (str, optional): The table name to prefix the key with. Defaults to None.
            expiration_time (int, optional): The expiration time in seconds. Defaults to None.
        """
        redis_key = key if not table else f"{table}:{key}"
        print(redis_key, self._encode_dict(value))
        self.connection.hmset(redis_key, self._encode_dict(value))  # type: ignore
        if expiration_time:
    ...
    def get_json(
        ...
        Returns:
            Optional[dict[str, Any]]: The retrieved JSON value, or None if not found.
        """
        return (
