...
def map_varnames(df, data_source, xwalk):
    # clean column names
    df.columns = df.columns.str.replace(" ","_").str.replace("[^\\d\\w]","").str.lower()
    assert all(xwalk["mpv"].isin(df.columns))

    # select and map column names
    name_map = {xwalk[data_source][i]: xwalk["common"][i] for i in range(xwalk.shape[0])} 
...
def make_single_table(table, dat, configs):
    ...
    cols = configs['tables'][table]['required'] + configs['tables'][table]['optional']
...
def make_all_tables():
    ...
    data_list = [make_tables_data_source(x, xwalk, configs) for x in data_sources]
    table_names = list(configs["tables"].keys())
