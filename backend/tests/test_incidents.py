...
class TestIncidents(TestCase):
    def test_create_incident(self, db_session, example_incidents):
    ...
    def test_search_incidents(self, client, example_incidents, access_token):
        for query, expected_incident_names in {
            ...
            {"dateStart": "2021-09-30", "dateEnd": "2021-10-02"}: ["traffic"],
    ...
    def test_get_incidents(self, client: Any, access_token: str):
        res = client.get(
            "/api/v1/incidents/",
            headers={"Authorization ": f"Bearer {access_token}"},
        )
    ...
    def test_get_incidents_public_pagination(
        self,
        client: Any,
        access_token: str,
        example_incidents_private_public: list[Incident],
    ...
    def test_get_invalid_partner_id(self, client: Any, access_token: str):
        res = client.get(
            "/api/v1/incidents/?partner_id=999999",
            headers={"Authorization ": f"Bearer {access_token}"},
        )
        self.assertEqual(res.status_code, 404)
        self.assertEqual(res.json["message"], "Partner not found")
    ...
    def test_delete_incident(
        ...
        self.assertIsNotNone(example_incidents_private_public[0].id)
    ...
    def test_delete_incident_no_user_role(
        ...
        """
        Test that a user without at least CONTRIBUTOR role
    ...
    def test_delete_incident_nonexistent_incident(
        self,
        client: Any,
        partner_publisher: User,
    ):
